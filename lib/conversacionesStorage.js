/**
 * Dani escribe en Firestore; el Director solo lee la colección conversaciones.
 *
 * .env:
...
async function logMensaje({
  ...
  if (conversacionId) {
    const ref = firestore.collection('conversaciones').doc(conversacionId);
  ...
  await firestore.collection('conversaciones').doc(id).set({
    ...doc,
    updatedAt: FieldValue.serverTimestamp(),
  });
  return doc;
